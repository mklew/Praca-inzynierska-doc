...
umowy do umów biur podróżnicznych. Klient korporacyjny to pracownik firmy, 
...
W brażny hotelarskiej występuję pojęcie \emph{Rack rate}.
...
\begin{itemize}
 ...
 \item romantyczny walentynowy weekend możesz spędzić w naszym hotelu. 
 Bukiet róż dla wybranki w dniu przyjazdu oraz romantyczna kolacja wieczorem 
 w naszej resturacji, dodatkowo (\dots) Tylko 549zł!
...
branży podróżniczej, której głownym celem jest stworzenie struktur dla 
...
Porównamy 4 systemy hotalarskie:
...
\begin{itemize}
  \item Rezerwacja - wsparcie systemu w zakresie
    \begin{itemize}
      ...
      \item rezerwacji ręcznej dla przychodniego klienta\footnote{Walk-ins}
      ...
      \item wparcie dla gwarantowanej, niegwarantowanej i tymczasowej rezerwacji
      \item możliwość konfiguracji godziny wygaśnięcia tymczasowej rezerwacji
      \item automatyczne kasowanie rezerwacji jeśli nie została potwierdzona przed określonym terminem
      \item rozdzielenia rezerwacji na dwie i osobnych modyfikacji na rozdzielonych rezerwacjach
      \item przesunięcia rezerwacji
      \item odwołania rezerwacji 
      \item dołączenia pokoju do rezerwacji
      \item zmiany pokoju rezerwacji przed pobytem
      \item zmiany pokoju w rezerwacji w trakcie pobytu
      ...
      \item każda rezerwacja powinna mieć swój unikalny numer
      ...
      \item usługa powinna mieć:
        \begin{itemize}
          ...
          \item sposób realizacji usługi:
            \begin{itemize}
              \item codziennie
              \item codzienie oprócz dnia przyjazdu
        ...
        \end{itemize}
    \end{itemize}
    \item wsparcia dla zarządzania produktami
  ...
  \item Zarządzanie pakietami\footnote{ang. Package management}
    ...
    \end{itemize}
  \item Recepcja\footnote{ang. Front office}
    \begin{itemize}
      \item system powinien wyświetlać grafik z obłożeniem hotelu
      \item grafik powinien być dostępny w różnych okresach czasowych: dany dzień, tydzień, dwa tygodnie, miesiąc
      \item system powinien wyświetlać statystyki:
        \begin{itemize}
          \item ilu gościu w hotelu
          \item ilu przyjeżdża
          \item ilu wyjeżdza
          \item ile pokoi wolnych
          \item ile pokoi zajętych
          \item ile pokoi do sprzątnięcia
        \end{itemize}
      \item recepcjonista powinien mieć wgląd w historie gościa
      \item system powinien wyświetlać przyjeżdzających gości w danym dniu
      \item system powinien wyświetlać wyjeżdzających gości w danym dniu
      \item umożliwienie zakwaterowania gościa\footnote{ang. Check-in}
      \item umożliwienie wykwaterowania gościa\footnote{ang. Check-out}
      \item system powinien wyświetlać ostatnio dokonane rezerwacje
      \item system powinien wyświetlać listę pokoi wraz z ich dostępnością i statusami
      \item  
    \end{itemize}
  \item Służba pięter\footnote{ang. Housekeeping}
\end{itemize}
...
\begin{itemize}
  \item numer rezerwacji nie może być kolejną liczbą z sekwencji
  \item system powinien archiwizować rezerwacje, nigdy usuwać
